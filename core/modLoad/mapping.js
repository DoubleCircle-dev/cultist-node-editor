 ...
 *    而要提取成节点或变量节点时，用 `materialize` 声明：
 *
 *    | materialize.as | 含义                                                       |
 *    | -------------- | ---------------------------------------------------------- |
 *    | `node`         | 提取成正式节点（`type` 是它的类别，如 slots / decks）        |
 *    | `tool`         | 提取成**变量节点**（`table` = 字典形态 / `list` = 列表形态，值留在节点里） |
 *
 *    两种都由 `toData` 落地：`node` 建正式子节点，`tool` 建 `role:'tool'` 的节点
 *    （自带 `tool` 描述符）；**怎么把变量画出来是前端的事**，后端只给模型与数据。
 *
 *    ⚠️ **术语（前后端统一）**：`role: 'tool'` 是**工具节点**这个大类
 *    （不是数据条目、不写数据文件）；其中持有值的叫**变量节点** ——
 *    标量 `text` / `number` / `images` + 字典 / 列表形态的 `table` / `list`；
 *    `container` / `danglingPort` / `previewNode` 等**不持有变量值**的节点仍属工具节点。
 *    契约字段名（`role` / `materialize.as` / `tool` 描述符）保持不变。
 *
